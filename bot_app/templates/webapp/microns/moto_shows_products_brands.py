    ...
    if brand_name:
        ...
        if await sync_to_async(products.exists)():  # Проверяем наличие продуктов
            ...
            for product in await sync_to_async(list)(products):  # Преобразуем QuerySet в список асинхронно
                ...
                # Сохраняем данные о продукте в user_data
                ...
                quantity = context.user_data["quantity"]  # Получаем текущее количество
                sklad_btn = create_reply_sklad_btn(quantity)  # Создаем кнопки для управления количеством
